...
if [ -n "${HELM_PLUGIN_NO_INSTALL_HOOK:-}" ]; then
...
on_exit() {
    ...
    if [ ${exit_code} -ne 0 ]; then
        echo "helm-ecr install hook failed. Please remove the plugin using 'helm plugin remove ecr' and install again." > /dev/stderr
    fi
    exit ${exit_code}
}
trap on_exit EXIT
...
echo "Downloading and installing helm-ecr ${version} ..."
...
mv "releases/v${version}/bin/helmecr" "bin/helmecr"
exit 0
